...
- Emergency Alerts: mirrors the Wireless Emergency Alert system. When NWS marks a product for delivery to phones — the full-screen "Emergency Alert" that overrides silent mode — the bot posts its own message: an `@everyone` ping, a full-size heading carrying the event and the shelter instruction, a yellow embed (gold, a shade brighter than the advisory-tier issuance notice it can sit beside), and the phone alert's own wording (`CMAMlongtext`) underneath, so it reads as the same message members just got. The trigger is NWS's WEA tagging (`WEAHandling` with CMAS unblocked, `thunderstormDamageThreat: DESTRUCTIVE`, any `tornadoDamageThreat`, `flashFloodDamageThreat: CATASTROPHIC`), backed by an event-name and Extreme-severity floor — the tags are dropped from continuation products, so an alert first seen mid-life would otherwise look untagged. Verified against Howard County's 2026-08-10 outbreak: it fires for the 4:10 PM DESTRUCTIVE 80 mph warning and the 4:14 PM tornado warning, and stays quiet for all six ordinary severe thunderstorm warnings that afternoon, which NWS blocked from phones. A warning upgraded to the phone tier under the same event name re-announces rather than being swallowed by dedupe. Unlike the routine issuance notice it ignores quiet hours (a 2 AM tornado warning is the one thing worth waking a server for), and like every notice it is deleted once the alert expires — a "TAKE SHELTER NOW" banner still at the top of the channel an hour after the tornado passed is its own kind of wrong. Two toggles in Settings > Feature Toggles: `Emergency Alerts` (the post) and `Emergency @everyone Ping` (the mention, which also needs the bot to hold *Mention Everyone* in the channel); the post survives with routine NWS notices switched off.
