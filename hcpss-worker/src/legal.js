...
export const PRIVACY_MD =
  ...
  "**3. Storage & Security** — Configuration data is stored in a secure database on the bot's hosting infrastructure with reasonable technical protections. The bot also writes an operational log of its own actions — what it posted, where, and which settings were changed (including the ID of whoever changed one); the hosting platform holds those lines for a few days and then discards them. Your staff can read them on the control panel's **System Logs** page, reachable only by a private link that expires after 30 minutes and shows no other server's activity. No system is 100% secure; data is stored and processed at your own risk.\n\n" +
  "**4. Retention & Deletion** — Server data is kept only while the bot remains in your server; removing the bot deletes it automatically or within a reasonable period. Admins may also erase it at any time with the `/mydata delete` command, or request manual deletion through the bot's support channel. The operational log in Section 3 is the exception: it ages out on the platform's own schedule and can't be deleted on request.\n\n" +
