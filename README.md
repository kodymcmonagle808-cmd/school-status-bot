...
- Emergency Alerts: when NWS pushes a Wireless Emergency Alert to phones — the full-screen tornado warning or destructive-wind alert that overrides silent mode — Discord gets a matching big yellow `@everyone` post quoting the same wording, with the shelter instruction up top. The trigger is NWS's own WEA tagging rather than a list of event names, so the ping fires exactly when phones go off and stays silent for the ordinary severe thunderstorm warnings NWS deliberately keeps off phones. Ignores quiet hours, re-fires if a warning is upgraded to the phone tier mid-life, and is removed within 5 minutes of the alert expiring like every other notice; both the alert and the `@everyone` on it are separately toggleable.
