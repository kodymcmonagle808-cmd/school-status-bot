...
- Emergency Alerts: the act-now tier gets its own big yellow `@everyone` post with the shelter instruction up top — tornado warnings, extreme wind, hurricane warnings, civil emergencies, and anything NWS rates Extreme (which is what a Flash Flood Emergency arrives as). It ignores quiet hours, is never auto-deleted, and stays deliberately rare so the ping keeps meaning something; both the alert and the `@everyone` on it are separately toggleable.
